...
class SocketInfo:
    ...
    bone_name: str          # Target bone to attach to
    offset: Tuple[float, float, float] = (0.0, 0.0, 0.0)  # Position offset (X, Y, Z)
    rotation: Tuple[float, float, float] = (0.0, 0.0, 0.0)  # Rotation offset in degrees (X, Y, Z)
...
class SemanticPart:
    ...
    name: str               # Part name (e.g., "Knight_Shoulder")
    category: str           # Category (armor, weapon, accessory, helmet, boots, etc.)
    style: str              # Style (heavy, light, scifi, fantasy, etc.)
    socket: SocketInfo      # Attachment socket info
    mesh_type: str = "primitive"  # "primitive" or "imported"
    mesh_data: Dict = None  # Mesh generation parameters
...
class KitbashLibrary:
    ...
    Provides categorized, style-aware equipment parts with socket/attachment data.
    AI can query by category and style to equip appropriate parts.
    """
    ...
    def _init_library(cls):
        ...
        cls._parts["Knight_Helmet"] = SemanticPart(
            ...
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["head"],
                offset=(0, 0, 0.08),
                rotation=(0, 0, 0),
                scale=0.12
            ),
            mesh_type="primitive",
            mesh_data={"type": "uv_sphere", "segments": 8, "rings": 6},
        ...
        cls._parts["Light_Hood"] = SemanticPart(
            ...
            mesh_type="primitive",
            mesh_data={"type": "cone", "vertices": 8, "radius1": 1.0, "depth": 0.8},
            tags=["rogue", "assassin", "light", "cloth"]
        )
        
        # ===== SHOULDER ARMOR =====
        cls._parts["Knight_Shoulder_L"] = SemanticPart(
            name="Knight_Shoulder_L",
            category="shoulder",
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["left_shoulder"],
                offset=(0.08, 0, 0.02),
                rotation=(0, 0, -15),
                scale=0.08
            ),
            mesh_type="primitive",
            mesh_data={"type": "uv_sphere", "segments": 6, "rings": 4},
            tags=["knight", "pauldron", "armor", "left"]
        )
        
        cls._parts["Knight_Shoulder_R"] = SemanticPart(
            name="Knight_Shoulder_R",
            category="shoulder",
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["right_shoulder"],
                offset=(-0.08, 0, 0.02),
                rotation=(0, 0, 15),
                scale=0.08
            ),
        ...
        cls._parts["Knight_Chestplate"] = SemanticPart(
            name="Knight_Chestplate",
            category="chest",
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["chest"],
                offset=(0, 0.08, 0),
                rotation=(0, 0, 0),
                scale=0.15
            ),
        ...
        cls._parts["Heavy_Boots_L"] = SemanticPart(
            ...
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["left_foot"],
                offset=(0, 0, -0.02),
                rotation=(0, 0, 0),
                scale=0.06
            ),
        ...
        cls._parts["Heavy_Boots_R"] = SemanticPart(
            ...
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["right_foot"],
                offset=(0, 0, -0.02),
                rotation=(0, 0, 0),
                scale=0.06
            ),
        ...
        cls._parts["SciFi_Boots_L"] = SemanticPart(
            ...
            mesh_type="primitive",
            mesh_data={"type": "cylinder", "vertices": 8, "radius": 0.5, "depth": 1.2},
        ...
        cls._parts["Knight_Gauntlet_L"] = SemanticPart(
            name="Knight_Gauntlet_L",
            category="gauntlet",
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["left_forearm"],
                offset=(0, 0, 0),
                rotation=(0, 0, 0),
                scale=0.04
            ),
            mesh_type="primitive",
            mesh_data={"type": "cylinder", "vertices": 6, "radius": 0.6, "depth": 1.5},
            tags=["knight", "gauntlet", "arm", "left"]
        )
        
        cls._parts["Knight_Gauntlet_R"] = SemanticPart(
            name="Knight_Gauntlet_R",
            category="gauntlet",
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["right_forearm"],
                offset=(0, 0, 0),
                rotation=(0, 0, 0),
                scale=0.04
            ),
        ...
        cls._parts["Knight_Sword"] = SemanticPart(
            ...
            style="heavy",
            socket=SocketInfo(
                bone_name=cls.BONE_SOCKETS["right_hand"],
                offset=(0, 0.05, 0),
                rotation=(90, 0, 0),
                scale=0.08
            ),
        ...
        cls._parts["Staff"] = SemanticPart(
            ...
            mesh_type="primitive",
            mesh_data={"type": "cylinder", "vertices": 6, "radius": 0.15, "depth": 4.0},
    ...
    # Category taxonomy - high-level categories map to specific part categories
    CATEGORY_TAXONOMY = {
    ...
    def query_parts(
        ...
        Supports high-level categories via taxonomy:
        - "armor" -> helmet, shoulder, chest, boots, gauntlet
        - "weapons" -> weapon, shield
        - "full_set" -> all armor + weapons
        """
        cls._init_library()
        ...
        if category:
            # Check if this is a taxonomy category
...
class KitbashEquipper:
    ...
    def create_primitive_mesh(mesh_data: Dict, name: str) -> bpy.types.Object:
        ...
        Avoids bpy.ops which requires context, uses bmesh primitives instead.
        """
        import bmesh
    ...
    def equip_part(
        ...
        Creates the mesh, positions it at the socket, and parents to bone.
        """
        ...
        # Apply socket transform with bone orientation
        socket = part.socket
        import math
        ...
        # Create socket offset rotation (convert degrees to radians)
        ...
        # Compose bone rotation with socket rotation
...
# Register equip_item tool
@tool("equip_item", "Equip semantic parts from Kitbash library to character")
def tool_equip_item(params: Dict) -> Dict:
    """
    Equip items from Kitbash library to character
    
    Params:
        category: Part category (helmet, shoulder, chest, boots, weapon, etc.)
        style: Style (heavy, scifi, light, magic)
        part_name: Specific part name (optional, overrides category/style query)
        color: RGB tuple for part color
        shader_style: Shader preset to apply
    """
    ...
    if part_name:
        ...
        # Query and equip matching parts
...
def tool_list_kitbash_parts(params: Dict) -> Dict:
    """
